...
async function registerCourse(studentId, tutorId, subjectId) {
  ...
  // Step 2: Check for ANY existing registration (to handle Unique Index on student+tutor+subject)
  const existingRegistration = await CourseRegistrationRepository.findOne({
    studentId,
    tutorId,
    subjectId
  ...
  if (existingRegistration) {
    // If already ACTIVE or PENDING -> Conflict
    if (existingRegistration.status === 'ACTIVE' || existingRegistration.status === 'PENDING') {
      throw new ConflictError('Bạn đã đăng ký môn học này với Tutor này rồi');
    }

    // If CANCELLED (or INACTIVE) -> Reactivate (Update existing doc)
    registration = await CourseRegistrationRepository.update(existingRegistration._id, {
      status: 'ACTIVE',
      registeredAt: new Date(),
      approvedAt: new Date(),
      approvedBy: 'SYSTEM'
    ...
    // BR-005 - INSTANT APPROVAL (Version 2.0)
    registration = await CourseRegistrationRepository.create({
      studentId,
      tutorId,
      subjectId,
  ...
  // Note: addRegisteredTutor should be idempotent (handle duplicates gracefully)
  await StudentRepository.addRegisteredTutor(student._id, tutor._id);

  // Step 5: Update Tutor statistics
  // Only increment if this is a NEW registration or a re-activation
  // (If we just reactivated, we should increment. If we created new, we increment.)
  ...
  await NotificationService.create(
    ...
    `Học sinh ${studentUser.fullName} (${student.mssv}) đã đăng ký môn ${subjectId}`,
    { relatedId: registration._id, relatedType: 'Registration' }
  );

  // Notify Student as well (since it's auto-approved)
  await NotificationService.create(
    studentUser._id,
    'REGISTRATION_APPROVED',
    'Đăng ký thành công',
    `Bạn đã đăng ký thành công môn ${subjectId} với Tutor.`,
    { relatedId: registration._id, relatedType: 'Registration' }
  );
