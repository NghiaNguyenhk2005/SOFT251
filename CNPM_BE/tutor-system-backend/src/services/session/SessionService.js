...
async function requestTutorSession(studentId, sessionData) {
  ...
  // Step 8: BR-008 - Send notification to tutor
  await NotificationService.create(
    tutor.userId,
    'SESSION_REQUEST',
    'Yêu cầu buổi học mới',
    `Student đã yêu cầu buổi học: ${sessionData.title}`,
    { relatedId: newSession._id, relatedType: 'TutorSession' }
  );
...
async function confirmSession(tutorId, sessionId) {
  ...
  // Step 6: BR-008 - Notify student
  const student = await StudentRepository.findById(session.requestedBy);
  if (student) {
    await NotificationService.create(
      student.userId,
      'SESSION_CONFIRMED',
      'Buổi học đã được xác nhận',
      `Tutor đã xác nhận buổi học: ${session.title}`,
      { relatedId: session._id, relatedType: 'TutorSession' }
    );
  }
...
async function rejectSession(tutorId, sessionId, reason = '') {
  ...
  // Step 5: BR-008 - Notify student
  const student = await StudentRepository.findById(session.requestedBy);
  if (student) {
    await NotificationService.create(
      student.userId,
      'SESSION_REJECTED',
      'Yêu cầu buổi học bị từ chối',
      `Tutor đã từ chối buổi học: ${session.title}. Lý do: ${reason}`,
      { relatedId: session._id, relatedType: 'TutorSession' }
    );
  }
...
async function bookAppointment(studentId, sessionId, notes = '') {
  ...
  // Step 8: BR-008 - Notify tutor
  const tutor = await TutorRepository.findById(session.tutorId);
  if (tutor) {
    await NotificationService.create(
      tutor.userId,
      'NEW_APPOINTMENT',
      'Có học sinh đăng ký buổi học',
      `Học sinh đã đăng ký buổi học: ${session.title}`,
      { relatedId: session._id, relatedType: 'TutorSession' }
    );
  }
...
async function cancelAppointment(studentId, appointmentId) {
  ...
  // Step 6: Notify Tutor
  const session = await TutorSessionRepository.findById(appointment.sessionId);
  if (session) {
    const tutor = await TutorRepository.findById(session.tutorId);
    if (tutor) {
      const student = await StudentRepository.findById(studentId);
      await NotificationService.create(
        ...
        'Học sinh hủy lịch hẹn',
        `Học sinh ${student ? student.fullName : ''} đã hủy lịch hẹn cho buổi học: ${session.title}`,
        { relatedId: session._id, relatedType: 'TutorSession' }
      );
    }
  }
